...
//========================================================================================================================================================
// Функции
export function deleteMoney(count, block, storrName = 'money') {
	let money = +localStorage.getItem(storrName);
	const items = document.querySelectorAll(block);
	localStorage.setItem(storrName, money - count);

	let moneyNew = getDigFormat(+localStorage.getItem(storrName));
...
export function addRemoveClass(block, className, delay) {
...
export function checkRemoveAddClass(block, className, item) {
	document.querySelectorAll(block).forEach(item => item.classList.remove(className));
	item.classList.add(className);
}

export function checkRemoveClass(block, className) {
	document.querySelectorAll(block).forEach(item => item.classList.remove(className));
}
...
export function addMoney(count, block, delay, delay_off) {
	let money = Math.floor(+localStorage.getItem('money') + count);
	setTimeout(() => {
		localStorage.setItem('money', money);
		document.querySelectorAll(block).forEach(el => el.textContent = getDigFormat(money));
...
export function addResource(count, block, delay, delay_off) {
	let money = Math.floor(+localStorage.getItem('resource') + count);
	setTimeout(() => {
		localStorage.setItem('resource', money);
		document.querySelectorAll(block).forEach(el => el.textContent = getDigFormat(money));
		document.querySelectorAll(block).forEach(el => el.classList.add('_anim-add-money'));
	}, delay);
	setTimeout(() => {
...
export function translToPercent(all, current) {
...
export function addNumberStorrage(name, number) {
...
export function getArrStorrage(name, sort = false) {
...
export function getRandomNumArr(mn, mx, length) {
...
// let randomArr = getRandomNumArr(1, 4, 2);
// let newRandArr = randomArr();
...
export function checkObjectRightCollision(rect1, rect2) {
...
export function checkObjectLeftCollision(rect1, rect2) {
...
export function checkObjectTopCollision(rect1, rect2) {
...
export function checkObjectDownCollision(rect1, rect2) {
