// Две кнопки - запуск и автозапуск
...
class Slot1 {
	...
	constructor(domElement: HTMLElement, config: IConfig) {
		...
		this.autoSpinButton = document.querySelector('[data-button="autospin-1"]') as HTMLElement;
		...
		this.autoSpinButton.addEventListener("click", () => {
			...
			if ((Number(localStorage.getItem('money')) > configSlot.minBet)) {
			...
			configSlot.timer = setInterval(function () {
				...
				if (oThis.casinoAutoSpinCount < 10 && (Number(localStorage.getItem('money')) >= configSlot.minBet)) {
					oThis.spin();
				} else if (configSlot.timer && oThis.casinoAutoSpinCount >= 10 && (Number(localStorage.getItem('money')) >= configSlot.minBet)) {
					clearInterval(configSlot.timer);
					configSlot.isAutMode = false;
				} else if (configSlot.timer) {
					clearInterval(configSlot.timer);
					// noMoney('.check');
					configSlot.isAutMode = false;
				}
